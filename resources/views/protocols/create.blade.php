...
<div class="space-y-8">
    <div class="flex items-center justify-between">
        <h1 class="text-4xl font-bold text-slate-900 dark:text-white">Create New Protocol</h1>
    ...
    <!-- Main Protocol Form -->
    <form action="{{ route('protocols.store') }}" method="POST" class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-8 space-y-8">
        @csrf

        @if ($errors->any())
            <div class="bg-red-100 dark:bg-red-900/50 border border-red-400 dark:border-red-800 text-red-700 dark:text-red-300 px-4 py-3 rounded relative text-sm" role="alert">
        ...
        <section class="space-y-6 border-b pb-6 border-slate-200 dark:border-slate-700">
            <h2 class="text-2xl font-semibold text-slate-800 dark:text-white">1. Protocol Information</h2>
            
            <!-- Title -->
            <div>
                <label for="title" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Protocol Title</label>
                <input type="text" name="title" id="title" required value="{{ old('title') }}"
                       class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 transition dark:bg-slate-700 dark:text-white"
        ...
        <section class="space-y-6">
            ...
            <button type="button" id="add-exercise-btn" class="flex items-center gap-2 px-4 py-2 border border-dashed border-emerald-500 text-emerald-600 dark:text-emerald-400 rounded-lg hover:bg-emerald-50 dark:hover:bg-slate-700 transition w-full justify-center">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function createExerciseSlot(index) {
            ...
            const exerciseSlotHtml = `
                ...
                <button type="button" class="remove-exercise-btn absolute top-3 right-3 text-red-500 hover:text-red-700 dark:hover:text-red-400 transition" title="Remove Exercise">
                ...
                <div>
                    ...
                    <select name="exercises[${index}][exercise_id]" required
                            class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 transition bg-white dark:bg-slate-800 dark:text-white">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="exercises[${index}][sets]" required min="1" value="3"
                               class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 transition dark:bg-slate-800 dark:text-white"
                               placeholder="e.g., 3">
                        <!-- Error placeholder removed to prevent PHP constant error -->
                    </div>
