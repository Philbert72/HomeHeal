...
<div class="space-y-8 max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-4xl font-extrabold text-slate-900 dark:text-white">Edit Protocol: {{ $protocol->title }}</h1>
    ...
    @if ($errors->any())
        <div class="bg-red-100 dark:bg-red-900/50 border border-red-400 dark:border-red-800 text-red-700 dark:text-red-300 px-4 py-3 rounded-lg relative text-sm" role="alert">
    ...
    <form action="{{ route('protocols.update', $protocol) }}" method="POST" class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 p-8 space-y-10">
        @csrf
        @method('PUT')

        <!-- Protocol Details Section -->
        <section class="space-y-6 border-b pb-8 border-slate-200 dark:border-slate-700">
            <h2 class="text-2xl font-bold text-indigo-600 dark:text-indigo-400 border-b-2 border-indigo-100 dark:border-indigo-900 inline-block pb-1">1. General Information</h2>
            
            <!-- Title -->
            <div>
                <label for="title" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Protocol Title</label>
                <input type="text" name="title" id="title" required 
                       value="{{ old('title', $protocol->title) }}"
                       class="w-full px-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition dark:bg-slate-700 dark:text-white"
        ...
        <section class="space-y-6">
            <h2 class="text-2xl font-bold text-indigo-600 dark:text-indigo-400 border-b-2 border-indigo-100 dark:border-indigo-900 inline-block pb-1">2. Exercise Prescription</h2>
            ...
            <button type="button" id="add-exercise-btn" class="flex items-center gap-2 px-6 py-3 border-2 border-dashed border-emerald-500 text-emerald-600 dark:text-emerald-400 font-semibold rounded-lg hover:bg-emerald-50 dark:hover:bg-slate-700 transition w-full justify-center">
        ...
        <div class="pt-8 border-t border-slate-200 dark:border-slate-700 flex justify-end items-center gap-4">
            ...
            <button type="button" 
                    ...
                    class="px-6 py-3 text-sm font-semibold text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/30 hover:bg-red-700 hover:text-white rounded-lg transition shadow-md">
                Delete Protocol
            </button>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function createExerciseSlot(index, data = {}) {
            ...
            slot.className = 'bg-white dark:bg-slate-700 p-6 rounded-xl border border-slate-200 dark:border-slate-600 shadow-md space-y-4 relative'; 
            ...
            const exerciseSlotHtml = `
                ...
                <div class="flex justify-between items-center mb-3 border-b border-slate-100 dark:border-slate-600 pb-2">
                    <h3 class="text-xl font-bold text-slate-800 dark:text-white">Exercise #${index + 1}</h3>
                ...
                <!-- Exercise Selector -->
                <div>
                    <label for="exercises[${index}][exercise_id]" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Select Exercise</label>
                    <select name="exercises[${index}][exercise_id]" required
                            class="w-full px-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition bg-white dark:bg-slate-800 dark:text-white">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mt-4">
                    <!-- Sets -->
                    <div>
                        <label for="exercises[${index}][sets]" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Sets</label>
                        <input type="number" name="exercises[${index}][sets]" required min="1" 
                               value="${currentSets}"
                               class="w-full px-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition dark:bg-slate-800 dark:text-white"
                               placeholder="e.g., 3">
                    </div>
                    
                    <!-- Reps -->
                    <div>
                        <label for="exercises[${index}][reps]" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Reps</label>
                    ...
                    <div class="col-span-1 sm:col-span-2 grid grid-cols-3 gap-2">
                        <div class="col-span-2">
                            <label for="exercises[${index}][resistance_value]" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Resistance Value (Optional)</label>
                        ...
                        <div class="col-span-1">
                            <label for="exercises[${index}][resistance_unit]" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Unit</label>
