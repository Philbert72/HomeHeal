...
<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        ...
        <p class="text-slate-600 dark:text-slate-400">Update your session details</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        ...
        <form action="{{ route('sessions.update', $session) }}" method="POST" class="p-8 space-y-8" x-data="sessionForm()">
            ...
            <div>
                ...
                >
                    @foreach($protocols as $protocol)
                        <option value="{{ $protocol->id }}" {{ old('protocol_id', $session->protocol_id) == $protocol->id ? 'selected' : '' }}>
                            {{ $protocol->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex gap-4 pt-6 border-t border-slate-200 dark:border-slate-700">
                <button type="submit" class="flex-1 py-3 bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-semibold rounded-lg hover:from-indigo-700 hover:to-blue-700 transition shadow-sm">
                    Update Session
                </button>
