...
<body class="bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 transition-colors">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @else
                        <!-- Mobile menu button -->
                        <button @click="mobileMenuOpen = !mobileMenuOpen" class="md:hidden p-2 text-slate-600 dark:text-slate-300 hover:text-slate-900 hover:bg-slate-100 rounded-lg">
                        ...
                        <div class="hidden md:block relative" x-data="{ open: false }">
                            <button @click="open = !open" class="flex items-center gap-2 px-3 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition">
                            ...
                            <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 py-1">
            ...
            <!-- Mobile Menu -->
            @auth
                <div x-show="mobileMenuOpen" x-transition class="md:hidden border-t border-slate-200 dark:border-slate-700 py-4">
                    <div class="space-y-1">
                        <!-- User info -->
                        <div class="px-4 py-3 border-b border-slate-200 dark:border-slate-700 mb-2">
                            <div class="flex items-center gap-3">
                        ...
                        <a href="{{ route('dashboard') }}" class="block px-4 py-2 text-slate-700 dark:text-slate-300 hover:bg-emerald-50 dark:hover:bg-slate-700 hover:text-emerald-600 rounded-lg mx-2">Dashboard</a>
                        
                        @if(Auth::user()->role === 'therapist')
                            <a href="{{ route('protocols.index') }}" class="block px-4 py-2 text-slate-700 dark:text-slate-300 hover:bg-emerald-50 dark:hover:bg-slate-700 hover:text-emerald-600 rounded-lg mx-2">Protocols</a>
                            <a href="{{ route('exercises.index') }}" class="block px-4 py-2 text-slate-700 dark:text-slate-300 hover:bg-emerald-50 dark:hover:bg-slate-700 hover:text-emerald-600 rounded-lg mx-2">Exercises</a>
                        @elseif(Auth::user()->role === 'patient')
                            <a href="{{ route('sessions.create') }}" class="block px-4 py-2 text-slate-700 dark:text-slate-300 hover:bg-emerald-50 dark:hover:bg-slate-700 hover:text-emerald-600 rounded-lg mx-2">Log Session</a>
                        @endif
    ...
    <main class="max-w-7xl mx-auto py-8 sm:px-6 lg:px-8 px-4">
        <!-- Flash Messages -->
        @if(session('success'))
            <div class="mb-6 p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg text-emerald-700 dark:text-emerald-400 flex items-center justify-between">
